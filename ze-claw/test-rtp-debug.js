...
let _multSumSamples = [], _multSumNoMult = 0, _multSumWithMult = 0, _clusterCount = 0;
function clusterPayout(cluster, bet) {
  ...
  _clusterCount++;
  if (multSum === 0) _multSumNoMult++;
  else { _multSumWithMult++; _multSumSamples.push(multSum); }
...
function runTumbleSync(bet, stats) {
  ...
  while (true) {
    ...
    for (const cluster of clusters) {
      ...
      stepWin += cw;
      stats.clusterCount++;
      // Track base pay (no multipliers) for diagnostic
...
function runSim(mode, N) {
  ...
  console.log('');
  console.log('══════════════════════════════════════════════════════');
console.log('multSum=0 (pay base*1):', _multSumNoMult, 'multSum>0:', _multSumWithMult);
if (_multSumSamples.length) {
  const avg = _multSumSamples.reduce((a,b)=>a+b,0)/_multSumSamples.length;
  _multSumSamples.sort((a,b)=>a-b);
  const med = _multSumSamples[Math.floor(_multSumSamples.length/2)];
  const p90 = _multSumSamples[Math.floor(_multSumSamples.length*0.9)];
  const p99 = _multSumSamples[Math.floor(_multSumSamples.length*0.99)];
  console.log('Avg multSum (when>0):', avg.toFixed(2), ' median:', med, ' p90:', p90, ' p99:', p99);
}
_multSumSamples=[]; _multSumNoMult=0; _multSumWithMult=0; _clusterCount=0;
  ...
  const avgBasePay = (stats.basePayOnly / N).toFixed(3);
  const avgMult    = stats.basePayOnly > 0 ? (stats.clusterWin / stats.basePayOnly).toFixed(4) : '0';
  const avgClSz    = stats.clusterCount > 0 ? (stats.totalTumbles / stats.clusterCount * 1).toFixed(3) : '0';
  console.log(`  Avg base pay/bonus       ${avgBasePay}   (cluster pays ignoring multipliers)`);
  console.log(`  Avg multiplier factor    ${avgMult}   (cluster win / base pay)`);
  console.log(`  Avg clusters/tumble      ${(stats.clusterCount / Math.max(1, stats.totalTumbles)).toFixed(3)}`);
  console.log('');
