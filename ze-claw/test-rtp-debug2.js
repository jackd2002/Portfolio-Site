...
let _wcCellSamples = [], _wcCellTotal = 0, _wcCellCount = 0;
function clusterPayout(cluster, bet) {
  ...
  const multSum = cluster.cells.reduce((s, { row, col }) => {
    const _wc = winCounts[row][col];
    _wcCellTotal += _wc;
    _wcCellCount++;
    if (_wc > 0) _wcCellSamples.push(_wc);
...
function runSim(mode, N) {
  ...
  console.log('');
  console.log('══════════════════════════════════════════════════════');
const _avgWc = _wcCellCount > 0 ? (_wcCellTotal / _wcCellCount).toFixed(4) : '0';
const _pctWithMult = _wcCellCount > 0 ? (_wcCellSamples.length / _wcCellCount * 100).toFixed(1) : '0';
console.log('Avg wc per cell in clusters: ' + _avgWc + '  % cells with wc>0: ' + _pctWithMult + '%');
_wcCellTotal = 0; _wcCellCount = 0; _wcCellSamples.length = 0;
